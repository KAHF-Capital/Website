...
function addSubscriber(subscriberData) {
  ...
  // Check if subscriber already exists (skip null fields)
  const existingIndex = subscribers.findIndex(
    s => s.stripeCustomerId === subscriberData.stripeCustomerId ||
         (subscriberData.phoneNumber && s.phoneNumber === subscriberData.phoneNumber) ||
         (subscriberData.email && s.email === subscriberData.email)
  );
  ...
  const subscriber = {
    ...
    preferences: {
      ...
      alertTime: subscriberData.alertTime || '09:46',
