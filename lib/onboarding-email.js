...
const { buildUnsubscribeUrl, getUnsubscribedSet } = require('./unsubscribe');
...
function wrap(html, unsubscribeUrl = null) {
  ...
  return `<!DOCTYPE html><html><body style="margin:0;padding:0;background:#f9fafb;font-family:-apple-system,BlinkMacSystemFont,'Segoe UI',Roboto,sans-serif">
    <table width="100%" cellpadding="0" cellspacing="0"><tr><td align="center" style="padding:32px 16px">
      ...
      <p style="color:#9ca3af;font-size:11px;margin:16px 0 0">KAHF Capital LLC · Educational only · Not investment advice${unsubLink}</p>
    </td></tr></table>
  </body></html>`;
}
...
async function sendOnboardingEmail({ email, step = 'welcome', source = 'unknown' }) {
  ...
  // Skip anyone on the suppression list (fail-open on Blob errors so a
  // transient outage doesn't silently kill the whole drip sequence).
  const unsubscribed = await getUnsubscribedSet();
  if (unsubscribed.has(String(email).toLowerCase())) {
    return { success: false, skipped: true, error: 'unsubscribed' };
  }
  ...
  try {
    const { data, error } = await getClient().emails.send({
      ...
      to: email,
      subject: tpl.subject,
      html: wrap(tpl.body({ email, source }), unsubscribeUrl),
      headers
    });
    if (error) {
