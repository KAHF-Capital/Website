...
// Build a checkout URL with optional email pre-fill, Firebase UID linking, and referral.
// Logged-in users pass their UID as client_reference_id so the webhook can grant
// Pro without relying on email match. Referrals stay in utm_content.
export function buildCheckoutUrl(baseUrl, options = {}) {
  if (!baseUrl) return '#';
  try {
    const url = new URL(baseUrl);
    if (options.email) url.searchParams.set('prefilled_email', options.email);
    // Prefer Firebase UID for durable linking; fall back to referral code only
    // when the buyer isn't signed in.
    if (options.uid) {
      url.searchParams.set('client_reference_id', options.uid);
    } else if (options.refCode) {
      url.searchParams.set('client_reference_id', options.refCode);
    }
    if (options.refCode) url.searchParams.set('utm_content', options.refCode);
