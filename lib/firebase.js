...
function getAuthErrorMessage(errorCode, rawMessage) {
  switch (errorCode) {
    ...
    case 'auth/configuration-not-found':
      return 'Firebase Authentication is not enabled. Go to Firebase Console → Authentication → Get started, then enable Google sign-in.';
    case 'auth/cancelled-popup-request':
      return 'Sign in was cancelled.';
    default:
      console.error(`Unhandled auth error: ${errorCode}`, rawMessage);
      return rawMessage || `Authentication error (${errorCode || 'unknown'}). Please try again.`;
  }
}
