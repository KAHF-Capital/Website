  ...
  try {
    ...
    if (!fs.existsSync(dataPath)) {
      ...
      if (!validateData(data)) {
        console.log('Invalid data structure detected, reinitializing...');
        const initialData = {
          trades: [],
          last_updated: null,
          analysis: {}
...
function loadData() {
  try {
    const dataPath = getDataPath();
    if (!fs.existsSync(dataPath)) {
      return { trades: [], last_updated: null, analysis: {} };
    }
...
// Save dark pool trade with validation
function saveDarkPoolTrade(trade) {
  try {
    // Validate trade data
    if (!trade || !trade.ticker || !trade.trade_date) {
    ...
    data.trades.push({
      ...
      exchange_id: trade.exchange_id || 0,
      trf_id: trade.trf_id || null,
