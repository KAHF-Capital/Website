...
async function sendDailyDigestEmail(toEmail, tickers, date, isQuietDay = false) {
  try {
    ...
    const rows = sorted.map(t => {
      ...
      return `<tr><td style="${b};font-weight:600;color:#e0e0e0">${t.ticker}</td><td style="${b};color:#00d4aa;font-weight:700;white-space:nowrap">${t.volume_ratio}x ${heat}</td><td style="${b};color:#a0a0b0;white-space:nowrap">${val}</td>${straddleCell}</tr>`;
    }).join('');
    ...
    const parts = [
      ...
      `<tr style="background:#141428"><th style="${th}">Ticker</th><th style="${th}">Vol Ratio</th><th style="${th}">Notional</th>${straddleHeader}</tr>`,
      rows,
      `</table>`,
