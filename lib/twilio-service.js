...
// When `newReads` is non-empty the read leads the SMS and the ticker list is
// trimmed to make room (SMS length is precious).
async function sendDailyDigest(phoneNumber, tickers, date, isQuietDay = false, newReads = []) {
  try {
    const client = getTwilioClient();
    const fromNumber = process.env.TWILIO_PHONE_NUMBER;
    ...
    const labels = { call: 'Long Call', put: 'Long Put', straddle: 'ATM Straddle' };
    const siteHost = (process.env.NEXT_PUBLIC_BASE_URL || 'https://www.kahfcapital.com').replace(/^https?:\/\//, '');

    const sorted = [...tickers].sort((a, b) => parseFloat(b.volume_ratio) - parseFloat(a.volume_ratio));

    const MAX_DISPLAY = hasReads ? 8 : 20;
    ...
    let body;
    if (hasReads) {
      const readLines = newReads.slice(0, 3).map(r =>
        `${r.ticker} ${labels[r.structure] || r.structure} $${r.strike} exp ${r.expiration} (edge ${r.asof_hit_rate}%)`
      ).join('\n');
      const moreReads = newReads.length > 3 ? `\n+${newReads.length - 3} more reads` : '';
      body =
